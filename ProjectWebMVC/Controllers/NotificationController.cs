...
namespace ProjectWebMVC.Controllers
{
    public class NotificationController : Controller
    {
        ...
        public NotificationController()
        {
            ...
            ScreeningUrl = "https://localhost:7269/api/Notification";
        }
        // GET: NotificationController
        public async Task<IActionResult> Index()
        {
            HttpResponseMessage respon = await _httpClient.GetAsync(ScreeningUrl);
            var strData = await respon.Content.ReadAsStringAsync();
            var options = new JsonSerializerOptions
            {
                PropertyNameCaseInsensitive = true,
            };
            List<Notification> screnninglist = JsonSerializer.Deserialize<List<Notification>>(strData, options);
            return View(screnninglist);
        }
        ...
        public async Task<IActionResult> Create(Notification n)
        {
            ...
            HttpResponseMessage respon = await _httpClient.PostAsync(ScreeningUrl, type);
